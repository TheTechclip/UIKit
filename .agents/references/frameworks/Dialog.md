...
`useDialog()` is backed by FunctionKit's named `buildContext` contract and must be called beneath a `<Dialog>` provider; it returns a non-null dialog context rather than an optional value.
